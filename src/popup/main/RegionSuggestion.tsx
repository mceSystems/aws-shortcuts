...
export function RegionSuggestion({ accounts }: Props) {
  const candidate = accounts
    .filter((a) => !a.defaultRegion)
    .map((a) => {
      const pick = pickRegionSuggestion(a.observedRegions, a.dismissedRegions);
      return pick ? { account: a, ...pick } : null;
    })
    .filter((x): x is { account: Account; region: string; hits: number } => Boolean(x))
    ...
    >
      ...
      <div className={styles.text}>
        <span className={styles.title}>
          Set <strong>{candidate.region}</strong> as default for{' '}
